...
/*
Hints: 
#478. Discuss the naive approach: merging names together when they are synonyms. How
would you identify transitive relationships? A == B, A == C, and C == D implies A
== D == B == C.

#493. The core part of the problem is to group names into the various spellings. From there,
figuring out the frequencies is relatively easy.

#512. One thing to try is maintaining a mapping of each name to its "true" spelling. You would
...
#537. An easier way of taking the above approach is to have each name map to a list of alter-
nate spellings. What should happen when a name in one group is set equal to a name in
another group?

#586. If each name maps to a list of its alternate spellings, you might have to update a lot of
...
#605. Instead, X, A, B, and C should map to the same instance of the set {X, A, B, C}. Y, D,
...
#675. We can treat adding X, Y as synonyms as adding an edge between the X node and the Y
