...
/*
Hints: 
#486. Consider first the algorithm for finding the closest distance if you will run the algorithm
only once. You should be able to do this in O(N) time, where N is the number of words
in the document.

#501. Adapt your algorithm for one execution of the algorithm for repeated executions. What
is the slow part? Can you optimize it?

#538. You could build a lookup table that maps from a word to a list of the locations where
each word appears. How then could you find the closest two locations?

#558. If you had a list of the occurrences of each word, then you are really looking for a pair
...
#633. Can you just iterate through both arrays with two pointers? You should be able to do it
